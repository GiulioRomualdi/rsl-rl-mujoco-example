...
GLOBAL_TRAJ_MANAGER = None

def set_global_trajectory_manager(mgr):
    global GLOBAL_TRAJ_MANAGER
    GLOBAL_TRAJ_MANAGER = mgr

def get_global_trajectory_manager():
    return GLOBAL_TRAJ_MANAGER
...
class ReferenceTrajectories:
    ...
    def __init__(
        ...
        if manager is None:
            manager = get_global_trajectory_manager()
        if manager is None:
            raise KeyError("ReferenceTrajectories: no TrajectoryManager provided or injected globally.")
        ...
        self.repeat_times = manager.get_repeat_times()
    ...
    def get_extend_refer(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Return the fully-expanded (43-DOF) reference state at the current cursor.
    
        - If the underlying trajectory is already 43-dimensional, returns it unchanged.
        - Otherwise, grabs the 37-dim raw state and calls expand_state() to
          insert the 6 patella-related DOFs in the correct mtp insertion slots.
    
        Returns:
            full_qpos (np.ndarray): Joint positions, shape (43,)
            full_qvel (np.ndarray): Joint velocities, shape (43,)
        """
        raw_qpos, raw_qvel = self.get_reference_trajectories()
        
        if raw_qpos.size == 43 and raw_qvel.size == 43:
            return raw_qpos, raw_qvel
        
        return self.expand_state(raw_qpos, raw_qvel)
        
    def expand_state(self, raw_qpos: np.ndarray, raw_qvel: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        Expand a 37-DOF reference trajectory state to the model’s 43-DOF state by inserting
        three patella-related DOFs per leg immediately after each mtp joint.
    
        If the input arrays already have length 43, they are returned unchanged.
    
        Args:
            raw_qpos (np.ndarray): 1D array of shape (37,) containing joint positions.
            raw_qvel (np.ndarray): 1D array of shape (37,) containing joint velocities.
    
        Returns:
            full_qpos (np.ndarray): 1D array of shape (43,) with extra DOFs inserted.
            full_qvel (np.ndarray): 1D array of shape (43,) with corresponding velocities.
        """
        TARGET_DIM = 43
        BASE_DIM = raw_qpos.size
        
        if BASE_DIM == TARGET_DIM:
            return raw_qpos, raw_qvel
        
        COEF = np.array([
            [-0.0108281, -0.0487847,   0.00927644,  0.0131673,   -0.00349673],   # translation2
            [ 0.0524192, -0.0150188,  -0.0340522,   0.0133393,   -0.000879151],  # translation1
            [ 0.010506 ,  0.0247615,  -1.31647   ,  0.716337 ,  -0.138302   ],  # rotation1
        ])

        def compute_beta(x: float, xdot: float) -> Tuple[np.ndarray, np.ndarray]:
            powers = np.array([x**i for i in range(COEF.shape[1])])
            angles = powers @ COEF.T
            dp = np.array([i * x**(i-1) if i > 0 else 0.0 for i in range(COEF.shape[1])])
            vels = (dp @ COEF.T) * xdot
            return angles, vels
        
        mp_idx_r = self.jnt_name['mtp_angle_r']
        mp_idx_l = self.jnt_name['mtp_angle_l']
        knee_idx_r = self.jnt_name['knee_angle_r']
        knee_idx_l = self.jnt_name['knee_angle_l']
        
        full_qpos = raw_qpos.copy()
        full_qvel = raw_qvel.copy()
    
        angles_r, vels_r = compute_beta(raw_qpos[knee_idx_r], raw_qvel[knee_idx_r])
        insert_r = mp_idx_r + 1
        full_qpos = np.insert(full_qpos, insert_r, angles_r)
        full_qvel = np.insert(full_qvel, insert_r, vels_r)
    
        angles_l, vels_l = compute_beta(raw_qpos[knee_idx_l], raw_qvel[knee_idx_l])
        insert_l = mp_idx_l + 1 + angles_r.size
        full_qpos = np.insert(full_qpos, insert_l, angles_l)
        full_qvel = np.insert(full_qvel, insert_l, vels_l)
    
        assert full_qpos.size == TARGET_DIM and full_qvel.size == TARGET_DIM, (
            f"Expanded state has wrong dim: {full_qpos.size}"
        )
